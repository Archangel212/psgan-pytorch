...
class PSGANGenerator(nn.Module):
    ...
    def __init__(self, conv_channels=[64, 512, 256, 128, 64, 3], kernel_size=5, local_noise_dim=40, global_noise_dim=20, periodic_noise_dim=4, spatial_size=6, hidden_noise_dim=60):
        ...
        for ch_index in range(1, len(conv_channels)-1):
            layers.append(nn.ConvTranspose2d(in_channels=conv_channels[ch_index-1], out_channels=conv_channels[ch_index], kernel_size=kernel_size, stride=2, padding=2))
        ...
        layers.append(nn.ConvTranspose2d(in_channels=conv_channels[-2], out_channels=conv_channels[-1], kernel_size=kernel_size, stride=2, padding=2))
    ...
    def generate_noise(self, batch_size, local_dim, global_dim, periodic_dim, spatial_size, tile=None):
        ...
        # set local noise
        Z[:, :local_dim] = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, local_dim, spatial_size, spatial_size))
        
        # set global noise
        if tile is None:
            Z_g = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
    ...
    def generate_noise_interpolation(self, batch_size, local_dim, global_dim, periodic_dim, spatial_size):
        ...
        upleft = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
        upright = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
        downleft = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
        downright = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
    ...
    def generate_noise_left2right_interpolation(self, batch_size, local_dim, global_dim, periodic_dim, spatial_size):
        ...
        left = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
        right = np.random.uniform(UNIFORM_RANGE_MIN, UNIFORM_RANGE_MAX, (batch_size, global_dim, 1, 1))
...
class PSGANDiscriminator(nn.Module):
    ...
    def __init__(self, conv_channels=[3, 64, 128, 256, 512, 1], kernel_size=5):
        ...
        layers = []
        for ch_index in range(1, len(conv_channels)-1):
            layers.append(nn.Conv2d(in_channels=conv_channels[ch_index-1], out_channels=conv_channels[ch_index], kernel_size=kernel_size, stride=2, padding=2))
            if ch_index != 1:
                layers.append(nn.BatchNorm2d(conv_channels[ch_index]))
            layers.append(nn.LeakyReLU(negative_slope=0.2, inplace=self.inplace_flag))
